...
public class Step3 {
    public static boolean run(Configuration conf, Map<String, String> paths) {
        ...
        try {
            ...
            FileInputFormat.setInputPaths(job, new Path(paths.get("Step3Input1")), new Path(paths.get("Step3Input2")));
    ...
    public static class Step3Mapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            } else if (flag.equals("Step1Output")) {
                // 用户得分矩阵
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            for (Text t : values) {
                ...
                if (line.startsWith("A")) {
                    String item = kv[0].substring(2);
                    itemMatrix.put(item, Integer.parseInt(kv[1]));
                } else if (line.startsWith("B")) {
                    String user = kv[0].substring(2);
                    userMatrix.put(user, Integer.parseInt(kv[1]));
                }
            }
